...
BOOL CDIBSectionLite::SetBitmap(LPCTSTR lpszResourceName)
{
	...
	if(bResult)
	{
		m_nBmpColumn = ((((GetWidth()*GetBitCount()+7)/8)+3)/4)*4;
	}
	//-------------------------------------------------------------
...
BOOL CDIBSectionLite::CreateEmptyDIB(int nWidth, int nHeight, int nBits)
{
    DeleteObject();
	if(nWidth<0||nHeight<0||nBits!=24) return FALSE;
	// Compute the number of colours in the colour table
	m_iColorTableSize = NumColorEntries(nBits);

	m_nBmpColumn = ((((nWidth*nBits+7)/8)+3)/4)*4;

	m_DIBinfo.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	m_DIBinfo.bmiHeader.biWidth = nWidth;
	m_DIBinfo.bmiHeader.biHeight = nHeight;
	m_DIBinfo.bmiHeader.biPlanes = 1;
	m_DIBinfo.bmiHeader.biBitCount = nBits;
	m_DIBinfo.bmiHeader.biCompression = BI_RGB;
	m_DIBinfo.bmiHeader.biSizeImage = m_nBmpColumn*nHeight;
//	m_DIBinfo.bmiHeader.biXPelsPerMeter = 11808;
//	m_DIBinfo.bmiHeader.biYPelsPerMeter = 11808;
	m_DIBinfo.bmiHeader.biClrUsed = m_iColorTableSize;
	m_DIBinfo.bmiHeader.biClrImportant = m_iColorTableSize;
...
BOOL CDIBSectionLite::CreateEmptyDIB(LPBITMAPINFO lpBitmapInfo)
{
    DeleteObject();
...
BOOL CDIBSectionLite::CreatePalette()
{
    ...
    for (int i=0; i < (int) m_iColorTableSize; i++)
    {
...
COLORREF CDIBSectionLite::GetPixel(int x,int y)
{
	unsigned char* pDibBits = (unsigned char*)m_ppvBits;

	int nPixelIndex = 0;
	for(int i = 0; i < (y + 1); ++i)
	{
		nPixelIndex += m_nBmpColumn;
	}
	nPixelIndex = GetImageSize() - nPixelIndex;
	int nNewX = 0;
	for(int i = 0; i < 3; ++i)
	{
		nNewX += x;
	...
	if ( GetBitCount() == 8 )
	{
		RGBQUAD PALETTE = m_DIBinfo.ColorTable()[pDibBits[nPixelIndex + x]];
		B = PALETTE.rgbBlue;
		G = PALETTE.rgbGreen;
		R = PALETTE.rgbRed;
	}
	...
	{
		B = int(pDibBits[nPixelIndex + nNewX + 0]);
		G = int(pDibBits[nPixelIndex + nNewX + 1]);
		R = int(pDibBits[nPixelIndex + nNewX + 2]);
	}
	return RGB(R,G,B);
}

BOOL CDIBSectionLite::SetPixel(int x,int y, COLORREF color)
{
	int nPixelIndex = 0;
	for(int i = 0; i < (y + 1); ++i)
	{
	...
	if ( GetBitCount() == 8 )
	{
		RGBQUAD *PALETTE = m_DIBinfo.ColorTable();
		for(int l = 0; l < 256; ++l)
		{
			if(PALETTE[l].rgbBlue == GetBValue(color)
				&&PALETTE[l].rgbGreen == GetGValue(color)
				&&PALETTE[l].rgbRed == GetRValue(color))
			{
				pDibBits[nPixelIndex + x] = l;
				break;
			}
		}
	}
	...
	{
		pDibBits[nPixelIndex + nNewX + 0] = GetBValue(color);
		pDibBits[nPixelIndex + nNewX + 1] = GetGValue(color);
		pDibBits[nPixelIndex + nNewX + 2] = GetRValue(color);
	}
	return TRUE;
}

BOOL CDIBSectionLite::SetPixel(int x,int y, BGRDATA color)
{
	int nPixelIndex = 0;
	for(int i = 0; i < (y + 1); ++i)
	{
	...
	if ( GetBitCount() == 8 )
	{
		...
		for(int l = 0; l < 256; ++l)
		{
			if(PALETTE[l].rgbBlue == color.B
				&&PALETTE[l].rgbGreen == color.G
				&&PALETTE[l].rgbRed == color.R)
			{
				pDibBits[nPixelIndex + x] = l;
				break;
			}
	...
	{
		(*((BGRDATA*)(pDibBits + nPixelIndex + nNewX))) = color;
	}
	return TRUE;
}
